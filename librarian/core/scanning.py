"""VirusTotal file scanning (moved from virustotal.py)."""
...
async def scan_file(file_path: str) -> dict | None:
    ...
    Returns a stats dict or None if VT is disabled or the file is too large.
    Stats keys: malicious, suspicious, undetected, harmless, timeout, failure,
    type-unsupported.
    """
    if not VT_API_KEY:
        return None
