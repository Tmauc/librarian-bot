...
- **Automatic update notifications** — on startup and every 24 hours, the bot checks the latest GitHub release and notifies all allowed users via Telegram if a newer version is available. Controlled by `GITHUB_REPO` (defaults to `owner/librarian-bot`). Notifications are deduplicated within a process run.
