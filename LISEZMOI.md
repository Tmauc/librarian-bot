...
1. Va sur la page des releases : **[github.com/owner/librarian-bot/releases](https://github.com/owner/librarian-bot/releases)**
...
Tu devrais maintenant avoir un dossier `librarian-bot` quelque part sur ton ordinateur.
...
1. Ouvre le dossier `librarian-bot`
...
3. Dans "Nom de l'application", écris `librarian-bot` et clique **Créer**
...
1. Dans le dossier `librarian-bot`, double-clique sur le fichier **`lancer.bat`**
...
```
ghcr.io/owner/librarian-bot:latest
```
...
- **"Toutes les sources sont indisponibles" / tous les téléchargements échouent** → dans la quasi-totalité des cas, c'est ta **box internet qui bloque les domaines de téléchargement** (`libgen.li`, etc.) au niveau DNS : elle les fait pointer vers `127.0.0.1` au lieu de leur vraie adresse, et le bot les refuse (protection anti-attaque). Solution : utiliser un DNS public comme Cloudflare (`1.1.1.1`) ou Google (`8.8.8.8`).
    - **Sur ton PC** : change les DNS de ta connexion — [tuto Windows — Le Crabe Info](https://lecrabeinfo.net/tutoriels/changer-les-dns-sur-windows/).
    - **En auto-hébergé (Docker)** : c'est déjà réglé — le `docker-compose.yml` fourni force `dns: [1.1.1.1, 8.8.8.8]`. Pour vérifier le blocage : `nslookup libgen.li` (s'il répond `127.0.0.1`, ta box filtre).
