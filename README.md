...
```bash
git clone https://github.com/owner/librarian-bot.git
cd librarian-bot
```
...
| Variable | Description |
|---|---|
| `GITHUB_REPO` | GitHub repository to watch for new releases, in `owner/repo` format. Defaults to `owner/librarian-bot`. Set to empty to disable. |
...
--- librarian-bot v1.2.1 ---
  ...
  Calibre        : ✓ ebook-convert trouvé
  Email / Kindle : ✓ activé
  Mises à jour   : ✓ owner/librarian-bot
...
A pre-built image is available at **`ghcr.io/owner/librarian-bot:latest`** — no need to build locally.
